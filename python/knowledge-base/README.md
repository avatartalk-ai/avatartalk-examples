# AvatarTalk Knowledge Base

A small FastAPI demo that lets you chat with an avatar whose answers are grounded in your own documents. On each start, it builds a temporary OpenAI vector store from files in a local folder and then:
...
- Place files in `python/knowledge-base/data` to seed the knowledge base
...
In `python/knowledge-base`, copy the example env and fill in your keys:
...
Place your reference files (PDF, CSV, text, etc.) in `python/knowledge-base/data` or point `KNOWLEDGE_BASE_DIRECTORY_PATH` to a different folder.
...
pip install -e .
knowledge-base-chat         # or: python -m knowledge_base
```
