...
- `python/expressive-webchat` – Natural voice conversations with automatic turn-taking (Deepgram Flux) and LLM-controlled emotional expressions via `/ws/continuous`. Ultra-low latency streaming.
...
- `nodejs/expressive-webchat` – Natural voice conversations with automatic turn-taking (Deepgram Nova 2) and LLM-controlled emotional expressions via `/ws/continuous`. Ultra-low latency streaming.
...
## Quick start (expressive-webchat – Python)

- `cd python/expressive-webchat`
- Ensure Python 3.10+ and install `uv`
- Create `.env` with `OPENAI_API_KEY`, `DEEPGRAM_API_KEY`, and `AVATARTALK_API_KEY`
- `./run.sh` or `uv run uvicorn src.app:app --reload --port 8080`
- Open `http://localhost:8080`
- Click "Start Session", allow microphone, and speak naturally

## Quick start (expressive-webchat – Node.js)

- `cd nodejs/expressive-webchat`
- Ensure Node.js 18+
- Create `.env` with `OPENAI_API_KEY`, `DEEPGRAM_API_KEY`, and `AVATARTALK_API_KEY`
