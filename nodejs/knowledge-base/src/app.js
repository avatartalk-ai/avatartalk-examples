...
import { responsesWithFileSearch, transcribeAudioBuffer } from './openai_client.js';
import { inference } from './avatartalk_client.js';
import { buildKnowledgeBase } from './data.js';
...
app.get('/', (req, res) => {
  res.render('index.html', {
    request: req, // compatibility stub
...
app.post('/chat', async (req, res) => {
  try {
    const kb = req.app.locals.kb;
    if (!kb?.vectorStoreId) {
      return res.status(503).json({ error: 'knowledge base not ready' });
    }
    ...
    const assistantText = await responsesWithFileSearch(messages, kb.vectorStoreId);
...
app.post('/voice', upload.single('audio'), async (req, res) => {
  try {
    ...
    const assistantText = await responsesWithFileSearch(msgs, kb.vectorStoreId);
...
async function main() {
  // Build knowledge base before listening
  ...
  const server = app.listen(settings.port, settings.host, () => {
    // eslint-disable-next-line no-console
    console.log(`AvatarTalk Knowledge Base listening on http://${settings.host}:${settings.port}`);
  });

  const shutdown = async () => {
    // eslint-disable-next-line no-console
    console.log('Shutting down...');
    try { await kb.shutDownVectorStore(); } catch {}
