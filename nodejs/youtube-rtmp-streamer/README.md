...
Small Node.js app that generates short English‑learning segments with OpenAI and streams them via AvatarTalk to a YouTube Live RTMP endpoint. It can also read live chat, adapt the next topic accordingly, and post generated responses back to the live chat.

## Quick Start
- Requirements: Node.js 18+ and npm.
- Install deps: `cd nodejs/youtube-rtmp-streamer && npm install`
- Set up YouTube OAuth2 credentials (see below).
- Create `.env` with your keys (see below).
- Start: `node src/main.js <YOUTUBE_VIDEO_ID>`

If you omit the video ID, set `YOUTUBE_LIVE_ID` in `.env` and run `node src/main.js`.
...
## Configuration (.env)
Set these environment variables in a `.env` file next to `src/main.js`:
...
## Run
- With explicit video ID: `node src/main.js <VIDEO_ID>`
- Using env fallback: `YOUTUBE_LIVE_ID=<VIDEO_ID> node src/main.js`
